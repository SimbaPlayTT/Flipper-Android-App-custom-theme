...
const val DEFAULT_SCREEN_BACKGROUND_COLOR = -0x73d7 // 0xFFFF8C29
...
object ScreenStreamFrameDecoder : LogTagProvider {
    ...
    fun emptyBitmap(backgroundColor: Int = DEFAULT_SCREEN_BACKGROUND_COLOR): Bitmap {
    ...
    fun decode(
        ...
        backgroundColor: Int = DEFAULT_SCREEN_BACKGROUND_COLOR
    ): FlipperScreenState.Ready? {
        ...
        fillBitmap(screen, bytes, orientation, backgroundColor)
    ...
    private fun fillBitmap(screen: Bitmap, bytes: ByteArray, orientation: ScreenOrientationEnum, backgroundColor: Int) {
        for (x in 0 until SCREEN_WIDTH) {
            for (y in 0 until SCREEN_HEIGHT) {
                val color = if (bytes.isPixelSet(x, y)) Color.BLACK else backgroundColor
