...
import com.flipperdevices.core.ui.theme.composable.color.desaturate

private const val PROGRESS_TINT_SATURATION_FACTOR = 0.7f
...
fun getThemedFlipperPallet(
    ...
    val overridden = if (customAccent != null) {
        val dPadColor = customDPadAccent ?: customAccent
        base.copy(
            accent = customAccent,
            dPadAccent = dPadColor,
            actionOnFlipperSubGhzEnable = customAccent,
            actionOnFlipperInfraredEnable = customAccent,
            actionOnFlipperSubGhzProgress = desaturate(customAccent, PROGRESS_TINT_SATURATION_FACTOR),
            actionOnFlipperInfraredProgress = desaturate(customAccent, PROGRESS_TINT_SATURATION_FACTOR),
            shareSheetBackgroundAction = customAccent,
            flipperScreenColor = customAccent,
            // Light theme keeps its neutral black outline by design; dark theme's outline is
            // always an accent-tinted glow, so only that one follows the custom color.
            screenStreamingBorderColor = if (isLight) {
                base.screenStreamingBorderColor
            } else {
