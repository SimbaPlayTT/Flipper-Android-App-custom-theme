...
fun FlipperTheme(
    ...
    val customAccent by themeViewModel.getCustomAccentColor().collectAsState()
    val customDPadAccent by themeViewModel.getCustomDPadAccentColor().collectAsState()
    val isLight = isLight(
        systemIsDark = isSystemInDarkTheme(),
        theme = theme
    )
    val pallet = getThemedFlipperPallet(isLight, customAccent, customDPadAccent)
    FlipperTheme(
        content = content,
        pallet = pallet,
        palletV2 = getThemedFlipperPalletV2(isLight, customAccent),
        isLight = isLight
    )
}
